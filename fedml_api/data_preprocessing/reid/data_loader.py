...
def preprocess_train_dataset(dataset, data_dir, batch_size):
    data_path = os.path.join(data_dir, dataset, 'pytorch')
    data_path = os.path.join(data_path, 'train_all')
    ...
    image_dataset = torchvision.datasets.ImageFolder(data_path)

    loader = torch.utils.data.DataLoader(
        ImageDataset(image_dataset.imgs, transform),
...
def load_partition_data_reid(datasets, data_dir, partition_method, client_number, batch_size):
    datasets = datasets.split(',')
    # print(f'{len(datasets)} : {client_number}')
    assert len(datasets) == client_number
    idxs_datasets_map = {i: datasets[i] for i in range(client_number)}

    # get local dataset
    data_local_num_dict = dict()
    train_data_local_dict = dict()
    test_data_local_dict = dict()
    local_classes_num_dict = dict()


    for client_idx in range(client_number):
        dataset = datasets[client_idx]

        train_data_local, image_dataset  = preprocess_train_dataset(dataset, data_dir, batch_size)
        logging.info("client_idx = %d, dataset = %s, local_sample_number = %d, batch_num_train_local = %d, local_classes_number = %d" % (client_idx, dataset, len(image_dataset), len(train_data_local), len(image_dataset.classes)))
        data_local_num_dict[client_idx] = len(image_dataset)
        train_data_local_dict[client_idx] = train_data_local
        local_classes_num_dict[client_idx] = len(image_dataset.classes)

        # print(image_dataset.imgs)
        # preprocess_test_dataset()
        # test_data_local_dict[client_idx] = test_data_local

    train_data_num = sum(data_local_num_dict.keys())
    test_data_num = 0
    train_data_global = None
    test_data_global = None
    
    return train_data_num, test_data_num, train_data_global, test_data_global, \
           data_local_num_dict, train_data_local_dict, test_data_local_dict, local_classes_num_dict
